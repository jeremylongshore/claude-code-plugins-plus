...
The exact reviewed head passed `ci-required`, `gitleaks`, `skill-conform`, the complete Validate
Plugins matrix, PR Pre-screen, Check Markdown Links, and all MiniMax Code Review jobs. Local
exact-head verification passed `pnpm run verify`, lint, typecheck, formatting, Ruff, Actionlint,
the audit-harness integrity check, 31/31 focused Python tests, and 36/36 Epic 1 measurement tests.
