...
**E2.6 count correction (2026-08-18).** The counts above are the 2026-06-30 decision-time
snapshot and had also drifted at time of writing (48 + 6 = 54, not the stated ~51). Measured
today: **64** externally-synced sources (**58** third-party + **6** `jeremylongshore/*`, per
`sources.yaml`, all 64 carrying `sources.lock.json` baselines per the E1.12 parity gate) of
**468** catalog plugins, leaving **404 (~86%)** in-repo Intent Solutions work. The decision and
its rationale are unchanged; only the scale numbers are corrected. Live values regenerate via
`pnpm run measure:e1` — do not re-quote this block's numbers as current without re-measuring.
