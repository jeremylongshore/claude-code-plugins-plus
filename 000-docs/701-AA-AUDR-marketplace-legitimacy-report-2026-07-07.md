...
4. `scan-synced-content` CHALLENGE — **meta false-positive**: it flagged the plugin's own `curl|wget` _detection regex_ as an outbound network call. The plugin makes no network calls. Narrow waiver.
