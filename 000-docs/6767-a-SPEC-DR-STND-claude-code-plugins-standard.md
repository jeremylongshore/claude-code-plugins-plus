> **SUPERSEDED–FROZEN (Epic 2 bead 2.1; 2026-08-15).** This historical standard is retained for citation and anchor compatibility; it is not authoritative. The governing platform standard is [727](727-AT-ARCH-master-modernization-blueprint.md). Its rules must not be used to reject current work: its plugin/marketplace contract predates the current canonical model, CI, mirror, and intake boundaries, and its `allowed-tools` examples do not establish the current schema. Do not edit below this banner; preserve the body and section anchors byte-for-byte.
