...
| 41  | `ci-required.needs` — documented vs actual **[RV]**                                | **19 documented / 17 named / 21 actual** (adds `commit-scope-check`, `codeowners-drift`, `generated-content-drift`, `doc-governance`)                                                                                                                                                                               | equal, machine-asserted                                                                           |
...
The first bounded E1.8 slice gates `skills-index.json`; the other three deterministic projections
remain E1.8 work, so that slice does not close this bead.
...
| 2.6  | Correct every documented number that disagrees with the code.                                                     | bug · P1     | Four verified drifts corrected **to measured truth, never the reverse**: schema 3.15.2→3.16.1; `ci-required` 19→21 enumerated; "all 317 agents A-grade"→347 files/253 errors/report-only; external-source count after 1.12. Each correction cites its command.               |
...
| 52  | Documented `ci-required` = 19, actual = 21                                                           | 2 / 10    | **2.7 (owner)**; 10.7 consumes                                      | P2  |
