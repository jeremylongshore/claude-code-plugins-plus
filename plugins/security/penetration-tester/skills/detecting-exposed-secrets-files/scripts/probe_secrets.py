...
PROBES = [
    ...
    (".git/HEAD", "Git repository .git/HEAD exposed",
     Severity.CRITICAL, r"^(ref:\s*refs/|[0-9a-f]{40})", "NIST 800-53 SC-28"),
    (".git/config", "Git repository .git/config exposed (may include remote credentials)",
     Severity.CRITICAL, r"\[remote\b", "NIST 800-53 SC-28"),
    (".git/index", "Git repository .git/index exposed",
     Severity.CRITICAL, r"^DIRC", "NIST 800-53 SC-28"),
    (".git/logs/HEAD", "Git repository ref log exposed",
     Severity.CRITICAL, r"[0-9a-f]{40}\s+[0-9a-f]{40}", "NIST 800-53 SC-28"),
    (".env", ".env file exposed (likely contains API keys, DB credentials)",
     Severity.CRITICAL, r"^[A-Z_][A-Z0-9_]*\s*=", "OWASP A05:2021"),
    (".env.production", ".env.production exposed",
     Severity.CRITICAL, r"^[A-Z_][A-Z0-9_]*\s*=", "OWASP A05:2021"),
    (".env.local", ".env.local exposed",
     Severity.CRITICAL, r"^[A-Z_][A-Z0-9_]*\s*=", "OWASP A05:2021"),
    (".aws/credentials", "AWS credentials file exposed",
     Severity.CRITICAL, r"\[default\]|aws_access_key_id", "CWE-200"),
    (".aws/config", "AWS config file exposed",
     Severity.CRITICAL, r"\[default\]|region\s*=", "CWE-200"),
    ("id_rsa", "SSH private key exposed",
     Severity.CRITICAL, r"BEGIN\s+(RSA|OPENSSH|EC|DSA)?\s*PRIVATE KEY", "CWE-321"),
    ("id_ed25519", "SSH ed25519 private key exposed",
     Severity.CRITICAL, r"BEGIN\s+OPENSSH\s+PRIVATE KEY", "CWE-321"),
    ("server.pem", "Server PEM key exposed",
     Severity.CRITICAL, r"BEGIN\s+(RSA\s+)?PRIVATE KEY", "CWE-321"),
    ("backup.sql", "SQL backup exposed",
     Severity.CRITICAL, r"CREATE\s+TABLE|INSERT\s+INTO", "CWE-538"),
    ("dump.sql", "SQL dump exposed",
     Severity.CRITICAL, r"CREATE\s+TABLE|INSERT\s+INTO", "CWE-538"),
    ("database.sql", "Database SQL exposed",
     Severity.CRITICAL, r"CREATE\s+TABLE|INSERT\s+INTO", "CWE-538"),
    ("backup.zip", "Archive backup.zip exposed",
     Severity.CRITICAL, None, "CWE-538"),
    ("backup.tar.gz", "Archive backup.tar.gz exposed",
     Severity.CRITICAL, None, "CWE-538"),
    ("dump.tar.gz", "Archive dump.tar.gz exposed",
     Severity.CRITICAL, None, "CWE-538"),

    # High - VCS metadata (less direct than .git but still source-of-truth-leaking)
    (".svn/entries", "Subversion .svn/entries exposed",
     Severity.HIGH, r"^\d+\s|^svn:", "NIST 800-53 SC-28"),
    (".svn/wc.db", "Subversion working copy DB exposed",
     Severity.HIGH, r"^SQLite", "NIST 800-53 SC-28"),
    (".hg/store/00manifest.i", "Mercurial repo manifest exposed",
     Severity.HIGH, None, "NIST 800-53 SC-28"),
    (".bzr/branch-format", "Bazaar branch format exposed",
     Severity.HIGH, r"Bazaar", "NIST 800-53 SC-28"),

    # Medium - useful enumeration for attacker
    (".DS_Store", "macOS .DS_Store exposed (reveals directory structure)",
     Severity.MEDIUM, r"^Bud1|^\x00\x00\x00\x01Bud1", "CWE-538"),
    ("Thumbs.db", "Windows Thumbs.db exposed",
     Severity.MEDIUM, None, "CWE-538"),
    ("phpinfo.php", "phpinfo() output exposed",
     Severity.MEDIUM, r"PHP Version|phpinfo\(\)", "CWE-200"),
    ("info.php", "PHP info exposed",
     Severity.MEDIUM, r"PHP Version|phpinfo\(\)", "CWE-200"),
    ("test.php", "Test PHP file exposed",
     Severity.MEDIUM, r"PHP Version|phpinfo\(\)", "CWE-200"),

    # Low - infrastructure metadata
    (".idea/workspace.xml", "JetBrains IDE config exposed",
     Severity.LOW, r"<project", "CWE-200"),
    (".vscode/settings.json", "VS Code config exposed",
     Severity.LOW, r"^\s*\{", "CWE-200"),
    (".gitlab-ci.yml", "GitLab CI config exposed",
     Severity.LOW, r"stages:|script:", "CWE-200"),
    (".github/workflows/", "GitHub Actions workflows dir exposed",
     Severity.LOW, None, "CWE-200"),
    ("Dockerfile", "Dockerfile exposed",
     Severity.LOW, r"^FROM\s+", "CWE-200"),
    ("docker-compose.yml", "docker-compose.yml exposed",
     Severity.LOW, r"^version:|services:", "CWE-200"),
    ("composer.json", "PHP composer.json exposed on web root",
     Severity.LOW, r'"name":\s*"', "CWE-200"),
    ("package.json", "Node package.json exposed on web root",
     Severity.LOW, r'"name":\s*"', "CWE-200"),
    ("requirements.txt", "Python requirements.txt exposed",
     Severity.LOW, r"^[a-zA-Z][a-zA-Z0-9_.-]*[=<>]", "CWE-200"),
    ("Gemfile", "Ruby Gemfile exposed",
     Severity.LOW, r"^source\s+['\"]https", "CWE-200"),
    ("config.yml", "Generic config.yml exposed",
     Severity.LOW, None, "CWE-200"),
    ("config.json", "Generic config.json exposed",
     Severity.LOW, r"^\s*\{", "CWE-200"),
    ("README.md", "README.md exposed on production web root",
     Severity.LOW, r"^#\s+", "CWE-200"),
]
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--paths-file", default=None,
                        help="Custom probe set (one path per line); replaces default")
    parser.add_argument("--check-only", action="store_true",
                        help="Skip body fingerprint check (treat any 200 as a finding)")
    ...
    if args.paths_file:
        ...
        probe_set = [(p.strip(), f"Custom path exposed: {p.strip()}",
                      Severity.MEDIUM, None, "custom") for p in paths if p.strip()]
    else:
        probe_set = PROBES
