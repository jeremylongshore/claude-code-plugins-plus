...
PUBLIC_SUFFIX_TLDS = {
    "com", "org", "net", "io", "co", "us", "uk", "de", "fr", "jp", "cn",
    "edu", "gov", "mil", "info", "biz", "name",
}
...
def _grab_chain_and_staple(host: str, port: int, timeout: float):
    ...
    try:
        proc = subprocess.run(
            cmd, input=b"", capture_output=True, timeout=timeout * 3,
        )
    ...
    try:
        ...
        if not ca_issuers:
            findings.append(Finding(
                ...
                target=target,
                detail=(
                    "AIA missing CA Issuers — clients can't fetch missing "
    ...
    for san in sans:
        ...
        if len(labels) <= 1 or labels[-1] in PUBLIC_SUFFIX_TLDS and len(labels) == 1:
            findings.append(Finding(
                ...
                remediation=(
                    "Reissue with narrower SAN(s) covering only the hostnames "
                    "actually served."
                ),
