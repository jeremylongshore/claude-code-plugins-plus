...
description: Use when the user asks what ads a competitor is running, wants a competitor ad teardown, or asks for competitive ad analysis. Trigger phrases include "what ads is X running", "tear down X's ad strategy", "competitor ad intelligence", "find ad angles we haven't tried", "reverse-engineer their paid funnel", or "audit the ad landscape for [category]". Scrapes the Meta Ad Library and Google Ads Transparency Center via web search (free public sources, no API keys), analyzes creative patterns and ad longevity, maps landing-page funnels, and outputs a strategic teardown with creative gaps, vulnerabilities, and counter-plays. Do NOT trigger for organic/SEO competitor research, website positioning analysis, or building comparison pages.
allowed-tools: WebSearch, WebFetch
version: 1.0.0
author: GooseWorks <himanshu@athina.ai>
license: MIT
compatibility: Uses web search and public ad libraries only — no API keys, credentials, or MCP servers required.
tags: [community, marketing, ads, competitive-intelligence]
---
