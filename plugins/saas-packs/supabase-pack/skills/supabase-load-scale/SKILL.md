...
async function getAllEventsSlow(orgId: string) {
  const { data, error } = await supabase
    .from('events')
    .select('id, event_type, payload, created_at')
...
//   WHERE org_id = '...' AND created_at > '2025-03-01';
...
CREATE TABLE public.events_p_2025_01 PARTITION OF public.events_partitioned
  FOR VALUES FROM ('2025-01-01') TO ('2025-02-01');
-- ... create partitions for each month with data
...
async function measureReplicaLag() {
  ...
  // Immediately read from replica and measure lag
