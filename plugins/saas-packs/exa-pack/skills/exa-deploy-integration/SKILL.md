...
description: |
  Deploy Exa integrations to production platforms.
  Use when deploying Exa-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy exa", "exa production",
  "exa production deploy", "exa CI/CD".
...
Deploy Exa-powered applications as stateless API wrappers — serverless functions
that receive user requests, call the Exa model API, and stream responses back.
...
export default async function handler(req: Request) {
  const client = new Client({ apiKey: process.env.EXA_API_KEY });
