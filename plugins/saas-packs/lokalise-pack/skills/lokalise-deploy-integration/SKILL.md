...
description: |
  Deploy Lokalise integrations to Vercel, Fly.io, and Cloud Run platforms.
  ...
  "lokalise production deploy", "lokalise Cloud Run", "lokalise Fly.io".
...
## Overview
Deploy Lokalise-powered applications to popular platforms with proper secrets management.
...
### Secrets
```bash
# Set Lokalise secrets
fly secrets set LOKALISE_API_KEY=sk_live_***
fly secrets set LOKALISE_WEBHOOK_SECRET=whsec_***
...
gcloud run deploy $SERVICE_NAME \
  ...
  --set-secrets=LOKALISE_API_KEY=lokalise-api-key:latest
```
...
export function getLokaliseConfig(): LokaliseConfig {
  ...
  return {
    ...
    environment: env as LokaliseConfig['environment'],
    webhookSecret: process.env.LOKALISE_WEBHOOK_SECRET,
  };
}
```
...
case "$1" in
  vercel)
    vercel secrets add lokalise_api_key "$LOKALISE_API_KEY"
    vercel --prod
    ;;
  fly)
    fly secrets set LOKALISE_API_KEY="$LOKALISE_API_KEY"
    fly deploy
    ;;
esac
```
