...
- [ ] Error handling covers all Lokalise error types
...
kubectl set image deployment/lokalise-integration app=image:new --record
kubectl rollout pause deployment/lokalise-integration
...
kubectl rollout resume deployment/lokalise-integration
kubectl rollout pause deployment/lokalise-integration
sleep 300

# Complete rollout to 100%
kubectl rollout resume deployment/lokalise-integration
kubectl rollout status deployment/lokalise-integration
```
...
```typescript
async function healthCheck(): Promise<{ status: string; lokalise: any }> {
  const start = Date.now();
  try {
    await lokaliseClient.ping();
    return { status: 'healthy', lokalise: { connected: true, latencyMs: Date.now() - start } };
  } catch (error) {
    return { status: 'degraded', lokalise: { connected: false, latencyMs: Date.now() - start } };
  }
}
```
...
### Immediate Rollback
```bash
kubectl rollout undo deployment/lokalise-integration
kubectl rollout status deployment/lokalise-integration
```
