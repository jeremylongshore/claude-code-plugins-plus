...
description: |
  Deploy Runway integrations to production platforms.
  Use when deploying Runway-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy runway", "runway production",
  "runway production deploy", "runway CI/CD".
...
Deploy Runway-powered applications as stateless API wrappers — serverless functions
that receive user requests, call the Runway model API, and stream responses back.
...
export default async function handler(req: Request) {
  const client = new Client({ apiKey: process.env.RUNWAY_API_KEY });
