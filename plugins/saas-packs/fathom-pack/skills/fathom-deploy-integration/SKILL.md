...
description: |
  Deploy Fathom integrations to production platforms.
  Use when deploying Fathom-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy fathom", "fathom production",
  "fathom production deploy", "fathom CI/CD".
...
Deploy Fathom-powered applications as stateless API wrappers — serverless functions
that receive user requests, call the Fathom model API, and stream responses back.
...
export default async function handler(req: Request) {
  const client = new Client({ apiKey: process.env.FATHOM_API_KEY });
