...
curl -s localhost:9090/api/v1/query?query=rate(groq_errors_total[5m])  # Prometheus port
...
# Capture metrics
curl "localhost:9090/api/v1/query_range?query=groq_errors_total&start=2h" > metrics.json  # Prometheus port
```
