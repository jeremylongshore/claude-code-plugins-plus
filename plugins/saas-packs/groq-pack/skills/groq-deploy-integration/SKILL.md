...
description: |
  Deploy Groq integrations to production platforms.
  Use when deploying Groq-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy groq", "groq production",
  "groq production deploy", "groq CI/CD".
...
Deploy Groq-powered applications as stateless API wrappers — serverless functions
that receive user requests, call the Groq model API, and stream responses back.
...
export default async function handler(req: Request) {
  const client = new Client({ apiKey: process.env.GROQ_API_KEY });
