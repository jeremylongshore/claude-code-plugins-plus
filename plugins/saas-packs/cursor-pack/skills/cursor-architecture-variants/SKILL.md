---
name: cursor-architecture-variants
description: |
  Choose and implement Cursor validated architecture blueprints for different scales.
  Use when designing new Cursor integrations, choosing between monolith/service/microservice
  architectures, or planning migration paths for Cursor applications.
  Trigger with phrases like "cursor architecture", "cursor blueprint",
  "how to structure cursor", "cursor project layout", "cursor microservice".
...
# Cursor Architecture Variants

## Overview
Three validated architecture blueprints for Cursor integrations.
...
- Synchronous Cursor calls in request path
- In-memory caching
...
app.post('/api/create', async (req, res) => {
  try {
    const result = await cursorClient.create(req.body);
    res.json(result);
  } catch (error) {
...
│   ├── middleware/
│   ├── queue/
│   │   └── cursor-processor.ts  # Async processing
│   └── index.ts
├── config/
│   └── cursor/
└── package.json
...
class CursorService {
  ...
  async createResource(data: CreateInput): Promise<Resource> {
    ...
    // Async follow-up
    await this.queue.enqueue('cursor.post-create', result);
...
```
cursor-service/              # Dedicated microservice
├── src/
│   ├── api/
│   │   ├── grpc/
│   │   │   └── cursor.proto
...
other-services/
├── order-service/       # Calls cursor-service
├── payment-service/
└── notification-service/
```

### Key Characteristics
- Dedicated Cursor microservice
...
class CursorAggregate {
  ...
  process(command: CursorCommand): void {
    // Domain logic
    const result = this.execute(command);

    // Emit domain event
    this.events.push(new CursorProcessedEvent(result));
  }
...
// Event handler
@EventHandler(CursorProcessedEvent)
class CursorEventHandler {
  async handle(event: CursorProcessedEvent): Promise<void> {
    // Saga orchestration
...
1. Extract Cursor code to service/
2. Add caching layer
3. Add background processing

Service Layer → Microservice:
1. Create dedicated cursor-service repo
...
- [Cursor Architecture Guide](https://docs.cursor.com/architecture)

## Next Steps
For common anti-patterns, see `cursor-known-pitfalls`.