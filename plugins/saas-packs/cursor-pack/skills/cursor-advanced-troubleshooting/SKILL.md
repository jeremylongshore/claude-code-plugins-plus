---
name: cursor-advanced-troubleshooting
description: |
  Apply Cursor advanced debugging techniques for hard-to-diagnose issues.
  Use when standard troubleshooting fails, investigating complex race conditions,
  or preparing evidence bundles for Cursor support escalation.
  Trigger with phrases like "cursor hard bug", "cursor mystery error",
  "cursor impossible to debug", "difficult cursor issue", "cursor deep debug".
...
# Cursor Advanced Troubleshooting

## Overview
Deep debugging techniques for complex Cursor issues that resist standard troubleshooting.
...
```bash
#!/bin/bash
# advanced-cursor-debug.sh

BUNDLE="cursor-advanced-debug-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BUNDLE"/{logs,metrics,network,config,traces}

# 1. Extended logs (1 hour window)
kubectl logs -l app=cursor-integration --since=1h > "$BUNDLE/logs/pods.log"
journalctl -u cursor-service --since "1 hour ago" > "$BUNDLE/logs/system.log"

# 2. Metrics dump
curl -s localhost:9090/api/v1/query?query=cursor_requests_total > "$BUNDLE/metrics/requests.json"
curl -s localhost:9090/api/v1/query?query=cursor_errors_total > "$BUNDLE/metrics/errors.json"

# 3. Network capture (30 seconds)
timeout 30 tcpdump -i any port 443 -w "$BUNDLE/network/capture.pcap" &

# 4. Distributed traces
curl -s localhost:16686/api/traces?service=cursor > "$BUNDLE/traces/jaeger.json"

# 5. Configuration state
kubectl get cm cursor-config -o yaml > "$BUNDLE/config/configmap.yaml"
kubectl get secret cursor-secrets -o yaml > "$BUNDLE/config/secrets-redacted.yaml"
...
async function diagnoseCursorIssue(): Promise<DiagnosisReport> {
  ...
  results.push(await testDNSResolution('api.cursor.com'));

  // Layer 3: TLS handshake
  results.push(await testTLSHandshake('api.cursor.com'));
...
```typescript
// Detect memory leaks in Cursor client usage
...
setInterval(() => {
  ...
  if (heapUsed.length > 60) { // 1 hour at 1/min
    ...
    if (trend > 100 * 1024 * 1024) { // 100MB growth
      console.warn('Potential memory leak in cursor integration');
    }
  }
}, 60000);
```
...
class CursorConcurrencyChecker {
...
### Evidence Attached
- [ ] Debug bundle (cursor-advanced-debug-*.tar.gz)
...
curl -v https://api.cursor.com/health 2>&1 | grep -E "(Connected|TLS|HTTP)"
```

## Resources
- [Cursor Support Portal](https://support.cursor.com)
- [Cursor Status Page](https://status.cursor.com)

## Next Steps
For load testing, see `cursor-load-scale`.