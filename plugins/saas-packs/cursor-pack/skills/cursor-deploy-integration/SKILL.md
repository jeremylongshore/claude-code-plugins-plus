...
description: |
  Deploy Cursor integrations to Vercel, Fly.io, and Cloud Run platforms.
  Use when deploying Cursor-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy cursor", "cursor Vercel",
  "cursor production deploy", "cursor Cloud Run", "cursor Fly.io".
...
## Overview
Deploy Cursor-powered applications to popular platforms with proper secrets management.
...
fly secrets set CURSOR_WEBHOOK_SECRET=whsec_***
...
gcloud run deploy $SERVICE_NAME \
  ...
  --set-secrets=CURSOR_API_KEY=cursor-api-key:latest
```
...
export function getCursorConfig(): CursorConfig {
  ...
  return {
    ...
    environment: env as CursorConfig['environment'],
    webhookSecret: process.env.CURSOR_WEBHOOK_SECRET,
  };
}
```
...
case "$1" in
  vercel)
    vercel secrets add cursor_api_key "$CURSOR_API_KEY"
    vercel --prod
    ;;
  fly)
    fly secrets set CURSOR_API_KEY="$CURSOR_API_KEY"
    fly deploy
    ;;
esac
```
