...
description: |
  Configure Speak local development with hot reload and testing.
  Use when setting up a development environment, configuring test workflows,
  or establishing a fast iteration cycle with Speak.
...
## Overview
Set up a fast, reproducible local development workflow for Speak.
...
import { SpeakClient } from '../src/speak/client';

describe('Speak Client', () => {
  it('should initialize with API key', () => {
    const client = new SpeakClient({ apiKey: 'test-key' });
...
### Mock Speak Responses
```typescript
vi.mock('@speak/sdk', () => ({
  SpeakClient: vi.fn().mockImplementation(() => ({
    // Mock methods here
  })),
}));
```
