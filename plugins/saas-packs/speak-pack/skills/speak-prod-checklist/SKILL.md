...
## Overview
Complete checklist for deploying Speak integrations to production.
...
```typescript
async function healthCheck(): Promise<{ status: string; speak: any }> {
  const start = Date.now();
  try {
    await speakClient.ping();
    return { status: 'healthy', speak: { connected: true, latencyMs: Date.now() - start } };
  } catch (error) {
    return { status: 'degraded', speak: { connected: false, latencyMs: Date.now() - start } };
  }
}
```
