...
async function processLargeDatabase(databaseId: string) {
  ...
  for await (const batch of paginateDatabase(databaseId)) {
    // Process each batch immediately — don't accumulate
...
// Store the last sync timestamp somewhere persistent (file, DB, env)
...
// Usage: sync only changes since last run
import fs from 'fs';
...
def paginate_database(database_id: str, filter_obj=None):
    """Generator that yields pages of results without loading all into memory."""
    cursor = None
    page_num = 0
...
# Use separate integrations for independent read/write workloads
...
function calculateCapacity(config: {
  ...
  cacheHitRate: number;     // 0-1, e.g., 0.8 for 80%
  ...
  const capacity = config.integrationTokens * 3; // 3 req/s per token

  console.log('=== Capacity Plan ===');
  console.log(`Raw reads/min: ${config.readsPerMinute}`);
  console.log(`After cache (${(config.cacheHitRate * 100)}% hit): ${Math.round(effectiveReads)}/min`);
  console.log(`Writes/min: ${config.writesPerMinute}`);
  console.log(`Total req/s needed: ${reqPerSecond.toFixed(1)}`);
  console.log(`Capacity (${config.integrationTokens} tokens): ${capacity} req/s`);
  console.log(`Headroom: ${((1 - reqPerSecond / capacity) * 100).toFixed(0)}%`);
  console.log(reqPerSecond > capacity ? 'WARNING: Over capacity!' : 'OK: Within limits');
}

// Example: 500 reads/min with 80% cache, 50 writes/min, 1 token
calculateCapacity({ readsPerMinute: 500, writesPerMinute: 50, cacheHitRate: 0.8, integrationTokens: 1 });
```
