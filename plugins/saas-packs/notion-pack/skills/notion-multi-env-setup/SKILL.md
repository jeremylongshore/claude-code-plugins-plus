...
aws secretsmanager get-secret-value --secret-id notion/production \
...
```bash
echo -n "ntn_prod_xxxxxxxxxxxxxxxxx" | \
  gcloud secrets create notion-token-prod --data-file=-
echo -n "abcdefab-cdef-abcd-efab-cdefabcdefab" | \
  gcloud secrets create notion-tasks-db-prod --data-file=-
...
// Seed test data only in dev/staging
...
function validateNotionConfig() {
  ...
  if (env === 'production' && token.includes('dev')) {
    throw new Error(
      'Production detected but NOTION_TOKEN contains "dev" — likely wrong token'
    );
  }
