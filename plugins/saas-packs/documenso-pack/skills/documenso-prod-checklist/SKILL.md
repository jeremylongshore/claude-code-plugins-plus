...
## Overview
Complete checklist for deploying Documenso integrations to production.
...
kubectl set image deployment/documenso-integration app=image:new --record
kubectl rollout pause deployment/documenso-integration
...
kubectl rollout resume deployment/documenso-integration
kubectl rollout pause deployment/documenso-integration
sleep 300

# Complete rollout to 100%
kubectl rollout resume deployment/documenso-integration
kubectl rollout status deployment/documenso-integration
```
...
```typescript
async function healthCheck(): Promise<{ status: string; documenso: any }> {
  const start = Date.now();
  try {
    await documensoClient.ping();
    return { status: 'healthy', documenso: { connected: true, latencyMs: Date.now() - start } };
  } catch (error) {
    return { status: 'degraded', documenso: { connected: false, latencyMs: Date.now() - start } };
  }
}
```
...
### Immediate Rollback
```bash
kubectl rollout undo deployment/documenso-integration
kubectl rollout status deployment/documenso-integration
```
