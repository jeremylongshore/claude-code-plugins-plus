...
# 2. Metrics dump
curl -s localhost:9090/api/v1/query?query=replit_requests_total > "$BUNDLE/metrics/requests.json"  # Prometheus port
...
curl -s localhost:16686/api/traces?service=replit > "$BUNDLE/traces/jaeger.json"  # Jaeger UI port
