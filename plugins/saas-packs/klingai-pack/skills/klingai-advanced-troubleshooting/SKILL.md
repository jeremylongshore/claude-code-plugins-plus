---
name: klingai-advanced-troubleshooting
description: |
  Apply Kling AI advanced debugging techniques for hard-to-diagnose issues.
  Use when standard troubleshooting fails, investigating complex race conditions,
  or preparing evidence bundles for Kling AI support escalation.
  Trigger with phrases like "klingai hard bug", "klingai mystery error",
  "klingai impossible to debug", "difficult klingai issue", "klingai deep debug".
...
# Kling AI Advanced Troubleshooting

## Overview
Deep debugging techniques for complex Kling AI issues that resist standard troubleshooting.
...
```bash
#!/bin/bash
# advanced-klingai-debug.sh

BUNDLE="klingai-advanced-debug-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BUNDLE"/{logs,metrics,network,config,traces}

# 1. Extended logs (1 hour window)
kubectl logs -l app=klingai-integration --since=1h > "$BUNDLE/logs/pods.log"
journalctl -u klingai-service --since "1 hour ago" > "$BUNDLE/logs/system.log"

# 2. Metrics dump
curl -s localhost:9090/api/v1/query?query=klingai_requests_total > "$BUNDLE/metrics/requests.json"
curl -s localhost:9090/api/v1/query?query=klingai_errors_total > "$BUNDLE/metrics/errors.json"

# 3. Network capture (30 seconds)
timeout 30 tcpdump -i any port 443 -w "$BUNDLE/network/capture.pcap" &

# 4. Distributed traces
curl -s localhost:16686/api/traces?service=klingai > "$BUNDLE/traces/jaeger.json"

# 5. Configuration state
kubectl get cm klingai-config -o yaml > "$BUNDLE/config/configmap.yaml"
kubectl get secret klingai-secrets -o yaml > "$BUNDLE/config/secrets-redacted.yaml"
...
async function diagnoseKling AIIssue(): Promise<DiagnosisReport> {
  ...
  results.push(await testDNSResolution('api.klingai.com'));

  // Layer 3: TLS handshake
  results.push(await testTLSHandshake('api.klingai.com'));
...
```typescript
// Detect memory leaks in Kling AI client usage
...
setInterval(() => {
  ...
  if (heapUsed.length > 60) { // 1 hour at 1/min
    ...
    if (trend > 100 * 1024 * 1024) { // 100MB growth
      console.warn('Potential memory leak in klingai integration');
    }
  }
}, 60000);
```
...
class Kling AIConcurrencyChecker {
...
### Evidence Attached
- [ ] Debug bundle (klingai-advanced-debug-*.tar.gz)
...
curl -v https://api.klingai.com/health 2>&1 | grep -E "(Connected|TLS|HTTP)"
```

## Resources
- [Kling AI Support Portal](https://support.klingai.com)
- [Kling AI Status Page](https://status.klingai.com)

## Next Steps
For load testing, see `klingai-load-scale`.