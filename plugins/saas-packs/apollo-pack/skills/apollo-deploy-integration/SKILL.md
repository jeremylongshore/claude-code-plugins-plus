...
description: |
  Deploy Apollo integrations to Vercel, Fly.io, and Cloud Run platforms.
  Use when deploying Apollo-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy apollo", "apollo Vercel",
  "apollo production deploy", "apollo Cloud Run", "apollo Fly.io".
...
## Overview
Deploy Apollo-powered applications to popular platforms with proper secrets management.
...
### Environment Setup
```bash
# Add Apollo secrets to Vercel
vercel secrets add apollo_api_key sk_live_***
vercel secrets add apollo_webhook_secret whsec_***
...
```bash
# Set Apollo secrets
fly secrets set APOLLO_API_KEY=sk_live_***
fly secrets set APOLLO_WEBHOOK_SECRET=whsec_***
...
export function getApolloConfig(): ApolloConfig {
  ...
  return {
    ...
    environment: env as ApolloConfig['environment'],
    webhookSecret: process.env.APOLLO_WEBHOOK_SECRET,
  };
}
```
...
case "$1" in
  vercel)
    vercel secrets add apollo_api_key "$APOLLO_API_KEY"
    vercel --prod
    ;;
  fly)
