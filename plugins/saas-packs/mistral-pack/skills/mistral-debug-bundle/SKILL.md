...
```bash
# SDK version
npm list @mistral/sdk 2>/dev/null >> "$BUNDLE_DIR/summary.txt"

# Recent logs (redacted)
grep -i "mistral" ~/.npm/_logs/*.log 2>/dev/null | tail -50 >> "$BUNDLE_DIR/logs.txt"
...
curl -s -o /dev/null -w "%{http_code}" https://api.mistral.com/health >> "$BUNDLE_DIR/summary.txt"
...
### Submit to Support
1. Create bundle: `bash mistral-debug-bundle.sh`
2. Review for sensitive data
3. Upload to Mistral AI support portal
