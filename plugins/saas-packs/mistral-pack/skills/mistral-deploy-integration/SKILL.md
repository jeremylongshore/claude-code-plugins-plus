...
### Environment Setup
```bash
# Add Mistral AI secrets to Vercel
vercel secrets add mistral_api_key sk_live_***
vercel secrets add mistral_webhook_secret whsec_***
...
### Secrets
```bash
# Set Mistral AI secrets
fly secrets set MISTRAL_API_KEY=sk_live_***
fly secrets set MISTRAL_WEBHOOK_SECRET=whsec_***
...
```typescript
// config/mistral.ts
interface Mistral AIConfig {
  apiKey: string;
  environment: 'development' | 'staging' | 'production';
  webhookSecret?: string;
}

export function getMistral AIConfig(): Mistral AIConfig {
  const env = process.env.NODE_ENV || 'development';

  return {
    apiKey: process.env.MISTRAL_API_KEY!,
    environment: env as Mistral AIConfig['environment'],
    webhookSecret: process.env.MISTRAL_WEBHOOK_SECRET,
  };
}
```
...
export async function GET() {
  const mistralStatus = await checkMistral AIConnection();
