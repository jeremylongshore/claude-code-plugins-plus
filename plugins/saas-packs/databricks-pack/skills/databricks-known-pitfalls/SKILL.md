---
name: databricks-known-pitfalls
description: |
  Identify and avoid Databricks anti-patterns and common integration mistakes.
  Use when reviewing Databricks code for issues, onboarding new developers,
  or auditing existing Databricks integrations for best practices violations.
  Trigger with phrases like "databricks mistakes", "databricks anti-patterns",
  "databricks pitfalls", "databricks what not to do", "databricks code review".
...
## Overview
Common mistakes and anti-patterns when integrating with Databricks.

## Prerequisites
- Access to Databricks codebase for review
...
### ❌ Anti-Pattern
```typescript
// User waits for Databricks API call
app.post('/checkout', async (req, res) => {
  const payment = await databricksClient.processPayment(req.body);  // 2-5s latency
  const notification = await databricksClient.sendEmail(payment);   // Another 1-2s
...
async function processCheckout(data) {
  const payment = await databricksClient.processPayment(data);
  await databricksClient.sendEmail(payment);
}
```
...
for (const item of items) {
  await databricksClient.process(item);  // Will hit rate limit
}
```
...
for (const item of items) {
  ...
  await limit(() => databricksClient.process(item));
}
```
...
try {
  await databricksClient.charge(order);
} catch (error) {
  if (error.code === 'NETWORK_ERROR') {
    await databricksClient.charge(order);  // Charged twice!
  }
}
```
...
app.post('/webhook',
  ...
  (req, res) => {
    const signature = req.headers['x-databricks-signature'];
    if (!verifyDatabricksSignature(req.body, signature)) {
...
const result = await databricksClient.get(id);
...
try {
  ...
  if (error instanceof DatabricksNotFoundError) {
    return null;
  }
  if (error instanceof DatabricksRateLimitError) {
...
const client = new DatabricksClient({
  ...
  baseUrl: 'https://api.databricks.com',  // Can't change for staging
});
```
...
const client = new DatabricksClient({
  ...
  baseUrl: process.env.DATABRICKS_BASE_URL || 'https://api.databricks.com',
});
```
...
### ❌ Anti-Pattern
```typescript
// When Databricks is down, every request hangs
for (const user of users) {
  await databricksClient.sync(user);  // All timeout sequentially
}
```
...
const breaker = new CircuitBreaker(databricksClient.sync, {
  timeout: 10000,
...
### ❌ Anti-Pattern
```typescript
// Entire feature broken if Databricks is down
const recommendations = await databricksClient.getRecommendations(userId);
...
let recommendations;
try {
  recommendations = await databricksClient.getRecommendations(userId);
} catch (error) {
  recommendations = await getFallbackRecommendations(userId);
  reportDegradedService('databricks', error);
}
