---
name: databricks-advanced-troubleshooting
description: |
  Apply Databricks advanced debugging techniques for hard-to-diagnose issues.
  Use when standard troubleshooting fails, investigating complex race conditions,
  or preparing evidence bundles for Databricks support escalation.
  Trigger with phrases like "databricks hard bug", "databricks mystery error",
  "databricks impossible to debug", "difficult databricks issue", "databricks deep debug".
...
## Overview
Deep debugging techniques for complex Databricks issues that resist standard troubleshooting.
...
```bash
#!/bin/bash
# advanced-databricks-debug.sh

BUNDLE="databricks-advanced-debug-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BUNDLE"/{logs,metrics,network,config,traces}

# 1. Extended logs (1 hour window)
kubectl logs -l app=databricks-integration --since=1h > "$BUNDLE/logs/pods.log"
journalctl -u databricks-service --since "1 hour ago" > "$BUNDLE/logs/system.log"

# 2. Metrics dump
curl -s localhost:9090/api/v1/query?query=databricks_requests_total > "$BUNDLE/metrics/requests.json"
curl -s localhost:9090/api/v1/query?query=databricks_errors_total > "$BUNDLE/metrics/errors.json"

# 3. Network capture (30 seconds)
timeout 30 tcpdump -i any port 443 -w "$BUNDLE/network/capture.pcap" &

# 4. Distributed traces
curl -s localhost:16686/api/traces?service=databricks > "$BUNDLE/traces/jaeger.json"

# 5. Configuration state
kubectl get cm databricks-config -o yaml > "$BUNDLE/config/configmap.yaml"
kubectl get secret databricks-secrets -o yaml > "$BUNDLE/config/secrets-redacted.yaml"
...
async function diagnoseDatabricksIssue(): Promise<DiagnosisReport> {
  ...
  results.push(await testDNSResolution('api.databricks.com'));

  // Layer 3: TLS handshake
  results.push(await testTLSHandshake('api.databricks.com'));
...
```typescript
// Detect memory leaks in Databricks client usage
...
setInterval(() => {
  ...
  if (heapUsed.length > 60) { // 1 hour at 1/min
    ...
    if (trend > 100 * 1024 * 1024) { // 100MB growth
      console.warn('Potential memory leak in databricks integration');
    }
  }
}, 60000);
```
...
class DatabricksConcurrencyChecker {
...
### Evidence Attached
- [ ] Debug bundle (databricks-advanced-debug-*.tar.gz)
...
curl -v https://api.databricks.com/health 2>&1 | grep -E "(Connected|TLS|HTTP)"
```

## Resources
- [Databricks Support Portal](https://support.databricks.com)
- [Databricks Status Page](https://status.databricks.com)

## Next Steps
For load testing, see `databricks-load-scale`.