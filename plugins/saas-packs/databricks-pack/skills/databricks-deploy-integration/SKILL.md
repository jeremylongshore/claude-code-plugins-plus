...
description: |
  Deploy Databricks integrations to production platforms.
  Use when deploying Databricks-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy databricks", "databricks production",
  "databricks production deploy", "databricks CI/CD".
...
Deploy Databricks integrations as data pipeline workers — persistent services that
...
import { Pool } from '@databricks/sdk';
...
### Deploy to Cloud Run
```bash
gcloud run deploy databricks-worker \
  --image gcr.io/$PROJECT_ID/databricks-worker \
  --min-instances=1 \
  --set-secrets=DATABRICKS_DATABASE_URL=databricks-db-url:latest
```
