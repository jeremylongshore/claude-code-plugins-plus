...
echo "DATABRICKS_API_KEY: ${DATABRICKS_API_KEY:+[SET]}" >> "$BUNDLE_DIR/summary.txt"
```

### Step 3: Gather SDK and Logs
```bash
# SDK version
npm list @databricks/sdk 2>/dev/null >> "$BUNDLE_DIR/summary.txt"

# Recent logs (redacted)
grep -i "databricks" ~/.npm/_logs/*.log 2>/dev/null | tail -50 >> "$BUNDLE_DIR/logs.txt"
...
curl -s -o /dev/null -w "%{http_code}" https://api.databricks.com/health >> "$BUNDLE_DIR/summary.txt"
...
### Submit to Support
1. Create bundle: `bash databricks-debug-bundle.sh`
2. Review for sensitive data
3. Upload to Databricks support portal
