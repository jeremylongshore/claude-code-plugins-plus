---
name: databricks-architecture-variants
description: |
  Choose and implement Databricks validated architecture blueprints for different scales.
  Use when designing new Databricks integrations, choosing between monolith/service/microservice
  architectures, or planning migration paths for Databricks applications.
  Trigger with phrases like "databricks architecture", "databricks blueprint",
  "how to structure databricks", "databricks project layout", "databricks microservice".
...
## Overview
Three validated architecture blueprints for Databricks integrations.
...
- Synchronous Databricks calls in request path
- In-memory caching
...
app.post('/api/create', async (req, res) => {
  try {
    const result = await databricksClient.create(req.body);
    res.json(result);
  } catch (error) {
...
│   ├── middleware/
│   ├── queue/
│   │   └── databricks-processor.ts  # Async processing
│   └── index.ts
├── config/
│   └── databricks/
└── package.json
...
class DatabricksService {
  ...
  async createResource(data: CreateInput): Promise<Resource> {
    ...
    // Async follow-up
    await this.queue.enqueue('databricks.post-create', result);
...
```
databricks-service/              # Dedicated microservice
├── src/
│   ├── api/
│   │   ├── grpc/
│   │   │   └── databricks.proto
...
other-services/
├── order-service/       # Calls databricks-service
├── payment-service/
└── notification-service/
```

### Key Characteristics
- Dedicated Databricks microservice
...
class DatabricksAggregate {
  ...
  process(command: DatabricksCommand): void {
    // Domain logic
    const result = this.execute(command);

    // Emit domain event
    this.events.push(new DatabricksProcessedEvent(result));
  }
...
// Event handler
@EventHandler(DatabricksProcessedEvent)
class DatabricksEventHandler {
  async handle(event: DatabricksProcessedEvent): Promise<void> {
    // Saga orchestration
...
1. Extract Databricks code to service/
2. Add caching layer
3. Add background processing

Service Layer → Microservice:
1. Create dedicated databricks-service repo
...
- [Databricks Architecture Guide](https://docs.databricks.com/architecture)

## Next Steps
For common anti-patterns, see `databricks-known-pitfalls`.