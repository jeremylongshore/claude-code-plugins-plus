...
description: |
  Analyze, implement, and verify GitHub Actions, FakeListChatModel, test gates, dry-run validators against LangChain 1.0.x baseline.
  Use when you need to github actions.
  Trigger with "langchain ci integration", "langchain python ci integration", "ci integration".
...
This skill covers: github actions, fakelistchatmodel, test gates, dry-run validators. The full v2.0 treatment will open with P05 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P05`, `P43`, `P44`, `P45`
