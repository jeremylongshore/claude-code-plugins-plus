...
description: |
  Analyze, implement, and verify LangSmith zero-code tracing, OTEL native export, custom metric callbacks against LangChain 1.0.x baseline.
  Use when you need to langsmith zero-code tracing.
  Trigger with "langchain observability", "langchain python observability", "observability".
...
This skill covers: langsmith zero-code tracing, otel native export, custom metric callbacks. The full v2.0 treatment will open with P26 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P26`, `P28`
