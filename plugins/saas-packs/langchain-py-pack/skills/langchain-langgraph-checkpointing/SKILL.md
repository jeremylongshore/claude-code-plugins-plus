...
description: |
  Analyze, implement, and verify MemorySaver, PostgresSaver, thread_id semantics, time-travel, state history against LangChain 1.0.x baseline.
  Use when you need to memorysaver.
  Trigger with "langchain langgraph checkpointing", "langgraph langgraph checkpointing", "langgraph checkpointing".
...
This skill covers: memorysaver, postgressaver, thread_id semantics, time-travel, state history. The full v2.0 treatment will open with P16 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P16`, `P17`, `P20`, `P40`, `P51`
