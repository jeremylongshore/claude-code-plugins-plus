...
description: |
  Analyze, implement, and verify Document loaders, RecursiveCharacterTextSplitter, semantic vs fixed chunking against LangChain 1.0.x baseline.
  Use when you need to document loaders.
  Trigger with "langchain data handling", "langchain python data handling", "data handling".
...
This skill covers: document loaders, recursivecharactertextsplitter, semantic vs fixed chunking. The full v2.0 treatment will open with P13 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P13`, `P49`, `P50`
