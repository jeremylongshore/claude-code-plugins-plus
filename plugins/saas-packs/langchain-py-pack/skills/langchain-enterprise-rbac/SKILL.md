...
description: |
  Analyze, implement, and verify Tenant isolation, per-tenant rate limits, role-scoped retrievers, audit logs against LangChain 1.0.x baseline.
  Use when you need to tenant isolation.
  Trigger with "langchain enterprise rbac", "langchain python enterprise rbac", "enterprise rbac".
...
This skill covers: tenant isolation, per-tenant rate limits, role-scoped retrievers, audit logs. The full v2.0 treatment will open with P33 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P33`
