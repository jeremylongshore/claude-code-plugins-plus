...
description: |
  Analyze, implement, and verify Real token accounting, model tiering, cache hit rates, per-tenant budget enforcement against LangChain 1.0.x baseline.
  Use when you need to real token accounting.
  Trigger with "langchain cost tuning", "langchain python cost tuning", "cost tuning".
...
This skill covers: real token accounting, model tiering, cache hit rates, per-tenant budget enforcement. The full v2.0 treatment will open with P01 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P01`, `P04`, `P10`, `P23`, `P25`, `P30`, `P31`, `P61`, `P62`
