...
description: |
  Analyze, implement, and verify Async callback handlers, SSE streaming, WebSocket, background event dispatch against LangChain 1.0.x baseline.
  Use when you need to async callback handlers.
  Trigger with "langchain webhooks events", "langchain python webhooks events", "webhooks events".
...
This skill covers: async callback handlers, sse streaming, websocket, background event dispatch. The full v2.0 treatment will open with P28 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P28`, `P46`, `P47`, `P48`, `P60`
