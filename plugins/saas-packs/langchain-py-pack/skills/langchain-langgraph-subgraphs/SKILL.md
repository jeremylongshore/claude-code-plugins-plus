...
description: |
  Analyze, implement, and verify Composing graphs, nested agent teams, shared state, subgraph boundaries against LangChain 1.0.x baseline.
  Use when you need to composing graphs.
  Trigger with "langchain langgraph subgraphs", "langgraph langgraph subgraphs", "langgraph subgraphs".
...
This skill covers: composing graphs, nested agent teams, shared state, subgraph boundaries. The full v2.0 treatment will open with P21 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P21`, `P28`
