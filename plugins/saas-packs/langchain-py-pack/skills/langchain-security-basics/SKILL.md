...
description: |
  Analyze, implement, and verify Prompt injection defenses, tool allowlisting, PII redaction, output validation against LangChain 1.0.x baseline.
  Use when you need to prompt injection defenses.
  Trigger with "langchain security basics", "langchain python security basics", "security basics".
...
This skill covers: prompt injection defenses, tool allowlisting, pii redaction, output validation. The full v2.0 treatment will open with P27 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P27`, `P32`, `P33`, `P34`, `P37`, `P50`, `P65`
