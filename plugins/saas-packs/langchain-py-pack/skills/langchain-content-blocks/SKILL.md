...
description: |
  Analyze, implement, and verify Typed AIMessage.content (text/tool_use/image), Claude tool_use iteration quirks against LangChain 1.0.x baseline.
  Use when you need to typed aimessage.content (text/tool_use/image).
  Trigger with "langchain content blocks", "langchain python content blocks", "content blocks".
...
This skill covers: typed aimessage.content (text/tool_use/image), claude tool_use iteration quirks. The full v2.0 treatment will open with P02 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P02`, `P58`, `P63`, `P64`
