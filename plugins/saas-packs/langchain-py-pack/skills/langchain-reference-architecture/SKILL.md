...
description: |
  Analyze, implement, and verify Layered design, LLM factory, chain registry, DI, tenant-scoped vector stores against LangChain 1.0.x baseline.
  Use when you need to layered design.
  Trigger with "langchain reference architecture", "langchain python reference architecture", "reference architecture".
...
This skill covers: layered design, llm factory, chain registry, di, tenant-scoped vector stores. The full v2.0 treatment will open with P22 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P22`, `P33`
