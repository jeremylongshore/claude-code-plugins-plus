...
description: |
  Analyze, implement, and verify LLM-specific SLOs, p95 latency triage, provider outage runbook, cost-overrun response against LangChain 1.0.x baseline.
  Use when you need to llm-specific slos.
  Trigger with "langchain incident runbook", "langchain python incident runbook", "incident runbook".
allowed-tools: Read
version: 2.0.0
license: MIT
...
This skill covers: llm-specific slos, p95 latency triage, provider outage runbook, cost-overrun response. The full v2.0 treatment will open with P10 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P10`, `P36`
