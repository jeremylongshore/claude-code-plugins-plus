...
description: |
  Analyze, implement, and verify pytest, FakeListChatModel, VCR fixtures, integration-test gating against LangChain 1.0.x baseline.
  Use when you need to pytest.
  Trigger with "langchain local dev loop", "langchain python local dev loop", "local dev loop".
...
This skill covers: pytest, fakelistchatmodel, vcr fixtures, integration-test gating. The full v2.0 treatment will open with P05 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.
