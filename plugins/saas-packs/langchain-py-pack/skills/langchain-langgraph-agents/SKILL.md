...
description: |
  Analyze, implement, and verify create_react_agent, prebuilt tool-calling agent, tools_condition, agent loop caps against LangChain 1.0.x baseline.
  Use when you need to create_react_agent.
  Trigger with "langchain langgraph agents", "langgraph langgraph agents", "langgraph agents".
...
This skill covers: create_react_agent, prebuilt tool-calling agent, tools_condition, agent loop caps. The full v2.0 treatment will open with P09 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P09`, `P10`, `P32`, `P41`, `P42`, `P63`
