...
description: |
  Analyze, implement, and verify Pydantic Settings env validation, dev/staging/prod isolation, secret backends against LangChain 1.0.x baseline.
  Use when you need to pydantic settings env validation.
  Trigger with "langchain multi env setup", "langchain python multi env setup", "multi env setup".
...
This skill covers: pydantic settings env validation, dev/staging/prod isolation, secret backends. The full v2.0 treatment will open with P37 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P37`
