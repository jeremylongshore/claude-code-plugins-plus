...
description: |
  Analyze, implement, and verify 1.0 middleware model, PII redaction, caching, retry middleware, ordering rules against LangChain 1.0.x baseline.
  Use when you need to 1.0 middleware model.
  Trigger with "langchain middleware patterns", "langchain python middleware patterns", "middleware patterns".
...
This skill covers: 1.0 middleware model, pii redaction, caching, retry middleware, ordering rules. The full v2.0 treatment will open with P10 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P10`, `P24`, `P25`, `P34`
