...
description: |
  Analyze, implement, and verify asyncio.Semaphore, token-bucket, exponential backoff, provider-specific limits against LangChain 1.0.x baseline.
  Use when you need to asyncio.semaphore.
  Trigger with "langchain rate limits", "langchain python rate limits", "rate limits".
...
This skill covers: asyncio.semaphore, token-bucket, exponential backoff, provider-specific limits. The full v2.0 treatment will open with P07 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P07`, `P29`, `P30`, `P31`
