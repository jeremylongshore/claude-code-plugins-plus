...
description: |
  Analyze, implement, and verify Deep Agents: planner + subagents + virtual filesystem + reflection loop against LangChain 1.0.x baseline.
  Use when you need to deep agents: planner + subagents + virtual filesystem + reflection loop.
  Trigger with "langchain deep agents", "langchain python deep agents", "deep agents".
...
This skill covers: deep agents: planner + subagents + virtual filesystem + reflection loop. The full v2.0 treatment will open with P51 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P51`, `P52`
