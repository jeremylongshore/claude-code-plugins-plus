...
description: |
  Analyze, implement, and verify stream_mode messages vs updates vs values, token-level streaming against LangChain 1.0.x baseline.
  Use when you need to stream_mode messages vs updates vs values.
  Trigger with "langchain langgraph streaming", "langgraph langgraph streaming", "langgraph streaming".
...
This skill covers: stream_mode messages vs updates vs values, token-level streaming. The full v2.0 treatment will open with P19 from the pain catalog — a real production failure mode — and walk through the exact fix with code pinned to LangChain 1.0.x.

Baseline: `langchain-core 1.0.x`, `langgraph 1.0.x`. Pain-catalog anchors: `P19`, `P46`, `P47`
