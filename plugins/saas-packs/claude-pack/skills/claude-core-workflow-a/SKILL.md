...
## Overview
This skill redirects to `claude-model-inference` which covers streaming, vision, structured output, and all Messages API patterns.

## Prerequisites
- Completed `claude-install-auth` setup
- `ANTHROPIC_API_KEY` configured

## Instructions

### Step 1: Use claude-model-inference instead
This skill has been replaced. The primary Anthropic workflow is the Messages API, covered in full by `claude-model-inference`.
...
## Output
- Redirected to `claude-model-inference`
- All Messages API patterns available there

## Error Handling
| Issue | Solution |
|-------|----------|
| Skill not found | Run `claude-model-inference` directly |
...
## Next Steps
Run `claude-model-inference` for the complete guide.
