# claude-core-workflow-a — One-Pager
...
| **What** | Redirect to `claude-model-inference` for all Messages API patterns |
| **When** | When you search for "anthropic workflow" or "claude main workflow" and need to find the right skill |

## Key Features

1. **Clear redirect** — Points directly to `claude-model-inference` as the canonical Messages API skill
