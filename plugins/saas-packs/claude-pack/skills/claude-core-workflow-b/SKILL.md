...
## Overview
This skill redirects to `claude-embeddings-search` which covers tool use (function calling), the agentic tool loop, and building Claude-powered agents.

## Prerequisites
- Completed `claude-model-inference`
- Understanding of JSON Schema for tool definitions

## Instructions

### Step 1: Use claude-embeddings-search instead
This skill has been replaced. The secondary Anthropic workflow is tool use / function calling, covered in full by `claude-embeddings-search`.
...
## Output
- Redirected to `claude-embeddings-search`
- Complete tool use patterns available there

## Error Handling
| Issue | Solution |
|-------|----------|
| Skill not found | Run `claude-embeddings-search` directly |
...
## Next Steps
Run `claude-embeddings-search` for the complete tool use guide.
