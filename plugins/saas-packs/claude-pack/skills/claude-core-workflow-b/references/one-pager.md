# claude-core-workflow-b — One-Pager
...
| **What** | Redirect to `claude-embeddings-search` for tool definitions, agentic loops, and tool result handling |
| **When** | When you search for "anthropic tools" or "claude function calling" and need to find the right skill |

## Key Features

1. **Clear redirect** — Points directly to `claude-embeddings-search` as the canonical tool use skill
