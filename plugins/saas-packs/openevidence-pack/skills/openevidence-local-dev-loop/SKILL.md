...
OPENEVIDENCE_TIMEOUT=60000  # 1 minute in ms
LOG_LEVEL=debug
EOF
...
OPENEVIDENCE_BASE_URL=https://api.sandbox.openevidence.com
OPENEVIDENCE_TIMEOUT=30000  # 30 seconds in ms
LOG_LEVEL=error
EOF
```
...
export function getClient(config?: Partial<ClientConfig>): OpenEvidenceClient {
  if (!clientInstance || config) {
    clientInstance = new OpenEvidenceClient({
      ...
      timeout: config?.timeout || parseInt(process.env.OPENEVIDENCE_TIMEOUT || '30000'),  # 30 seconds in ms
    });
  }
...
describe('OpenEvidence Integration', () => {
  ...
  }, 30000); // 30s timeout for API calls  # 30 seconds in ms
});
```
