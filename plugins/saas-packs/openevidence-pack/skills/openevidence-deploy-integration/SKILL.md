...
export async function GET() {
  const openevidenceStatus = await checkOpenEvidenceConnection();

  return Response.json({
    status: openevidenceStatus ? 'healthy' : 'degraded',
    services: {
      openevidence: openevidenceStatus,
    },
...
Use the platform CLI to deploy your application with OpenEvidence integration.

### Step 4: Verify Health
Test the health check endpoint to confirm OpenEvidence connectivity.

## Output
- Application deployed to production
- OpenEvidence secrets securely configured
...
- [OpenEvidence Deploy Guide](https://docs.openevidence.com/deploy)

## Next Steps
For webhook handling, see `openevidence-webhooks-events`.