...
description: |
  Deploy Ideogram integrations to production platforms.
  Use when deploying Ideogram-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy ideogram", "ideogram production",
  "ideogram production deploy", "ideogram CI/CD".
...
Deploy Ideogram-powered applications as stateless API wrappers — serverless functions
that receive user requests, call the Ideogram model API, and stream responses back.
...
export default async function handler(req: Request) {
  const client = new Client({ apiKey: process.env.IDEOGRAM_API_KEY });
