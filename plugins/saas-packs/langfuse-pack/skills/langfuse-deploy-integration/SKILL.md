...
description: |
  Deploy Langfuse integrations to Vercel, Fly.io, and Cloud Run platforms.
  ...
  "langfuse production deploy", "langfuse Cloud Run", "langfuse Fly.io".
...
## Overview
Deploy Langfuse-powered applications to popular platforms with proper secrets management.
...
### Environment Setup
```bash
# Add Langfuse secrets to Vercel
vercel secrets add langfuse_api_key sk_live_***
vercel secrets add langfuse_webhook_secret whsec_***
...
fly secrets set LANGFUSE_API_KEY=sk_live_***
fly secrets set LANGFUSE_WEBHOOK_SECRET=whsec_***
...
gcloud run deploy $SERVICE_NAME \
  ...
  --set-secrets=LANGFUSE_API_KEY=langfuse-api-key:latest
```
...
export function getLangfuseConfig(): LangfuseConfig {
  ...
  return {
    ...
    environment: env as LangfuseConfig['environment'],
    webhookSecret: process.env.LANGFUSE_WEBHOOK_SECRET,
  };
}
```
...
case "$1" in
  vercel)
    ...
    ;;
  fly)
    fly secrets set LANGFUSE_API_KEY="$LANGFUSE_API_KEY"
    fly deploy
    ;;
esac
```
