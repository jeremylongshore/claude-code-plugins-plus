---
name: openrouter-architecture-variants
description: |
  Choose and implement OpenRouter validated architecture blueprints for different scales.
  Use when designing new OpenRouter integrations, choosing between monolith/service/microservice
  architectures, or planning migration paths for OpenRouter applications.
  Trigger with phrases like "openrouter architecture", "openrouter blueprint",
  "how to structure openrouter", "openrouter project layout", "openrouter microservice".
...
# OpenRouter Architecture Variants

## Overview
Three validated architecture blueprints for OpenRouter integrations.
...
- Synchronous OpenRouter calls in request path
- In-memory caching
...
app.post('/api/create', async (req, res) => {
  try {
    const result = await openrouterClient.create(req.body);
    res.json(result);
  } catch (error) {
...
│   ├── middleware/
│   ├── queue/
│   │   └── openrouter-processor.ts  # Async processing
│   └── index.ts
├── config/
│   └── openrouter/
└── package.json
...
class OpenRouterService {
  ...
  async createResource(data: CreateInput): Promise<Resource> {
    ...
    // Async follow-up
    await this.queue.enqueue('openrouter.post-create', result);
...
```
openrouter-service/              # Dedicated microservice
├── src/
│   ├── api/
│   │   ├── grpc/
│   │   │   └── openrouter.proto
...
other-services/
├── order-service/       # Calls openrouter-service
├── payment-service/
└── notification-service/
```

### Key Characteristics
- Dedicated OpenRouter microservice
...
class OpenRouterAggregate {
  private events: DomainEvent[] = [];

  process(command: OpenRouterCommand): void {
    // Domain logic
    const result = this.execute(command);

    // Emit domain event
    this.events.push(new OpenRouterProcessedEvent(result));
  }
...
// Event handler
@EventHandler(OpenRouterProcessedEvent)
class OpenRouterEventHandler {
  async handle(event: OpenRouterProcessedEvent): Promise<void> {
    // Saga orchestration
...
1. Extract OpenRouter code to service/
2. Add caching layer
3. Add background processing

Service Layer → Microservice:
1. Create dedicated openrouter-service repo
...
- [OpenRouter Architecture Guide](https://docs.openrouter.com/architecture)

## Next Steps
For common anti-patterns, see `openrouter-known-pitfalls`.