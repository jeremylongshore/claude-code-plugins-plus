---
name: openrouter-advanced-troubleshooting
description: |
  Apply OpenRouter advanced debugging techniques for hard-to-diagnose issues.
  Use when standard troubleshooting fails, investigating complex race conditions,
  or preparing evidence bundles for OpenRouter support escalation.
  Trigger with phrases like "openrouter hard bug", "openrouter mystery error",
  "openrouter impossible to debug", "difficult openrouter issue", "openrouter deep debug".
...
# OpenRouter Advanced Troubleshooting

## Overview
Deep debugging techniques for complex OpenRouter issues that resist standard troubleshooting.
...
```bash
#!/bin/bash
# advanced-openrouter-debug.sh

BUNDLE="openrouter-advanced-debug-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BUNDLE"/{logs,metrics,network,config,traces}

# 1. Extended logs (1 hour window)
kubectl logs -l app=openrouter-integration --since=1h > "$BUNDLE/logs/pods.log"
journalctl -u openrouter-service --since "1 hour ago" > "$BUNDLE/logs/system.log"

# 2. Metrics dump
curl -s localhost:9090/api/v1/query?query=openrouter_requests_total > "$BUNDLE/metrics/requests.json"
curl -s localhost:9090/api/v1/query?query=openrouter_errors_total > "$BUNDLE/metrics/errors.json"

# 3. Network capture (30 seconds)
timeout 30 tcpdump -i any port 443 -w "$BUNDLE/network/capture.pcap" &

# 4. Distributed traces
curl -s localhost:16686/api/traces?service=openrouter > "$BUNDLE/traces/jaeger.json"

# 5. Configuration state
kubectl get cm openrouter-config -o yaml > "$BUNDLE/config/configmap.yaml"
kubectl get secret openrouter-secrets -o yaml > "$BUNDLE/config/secrets-redacted.yaml"
...
async function diagnoseOpenRouterIssue(): Promise<DiagnosisReport> {
  ...
  results.push(await testDNSResolution('api.openrouter.com'));

  // Layer 3: TLS handshake
  results.push(await testTLSHandshake('api.openrouter.com'));
...
```typescript
// Detect memory leaks in OpenRouter client usage
...
setInterval(() => {
  ...
  if (heapUsed.length > 60) { // 1 hour at 1/min
    ...
    if (trend > 100 * 1024 * 1024) { // 100MB growth
      console.warn('Potential memory leak in openrouter integration');
    }
  }
}, 60000);
```
...
class OpenRouterConcurrencyChecker {
...
### Evidence Attached
- [ ] Debug bundle (openrouter-advanced-debug-*.tar.gz)
...
curl -v https://api.openrouter.com/health 2>&1 | grep -E "(Connected|TLS|HTTP)"
```

## Resources
- [OpenRouter Support Portal](https://support.openrouter.com)
- [OpenRouter Status Page](https://status.openrouter.com)

## Next Steps
For load testing, see `openrouter-load-scale`.