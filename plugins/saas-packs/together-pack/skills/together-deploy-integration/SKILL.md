...
description: |
  Deploy Together AI integrations to production platforms.
  Use when deploying Together AI-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy together", "together production",
  "together production deploy", "together CI/CD".
...
Deploy Together AI-powered applications as stateless API wrappers — serverless functions
that receive user requests, call the Together AI model API, and stream responses back.
...
export default async function handler(req: Request) {
  const client = new Client({ apiKey: process.env.TOGETHER_API_KEY });
...
export async function GET() {
  const togetherStatus = await checkTogetherAIConnection();
