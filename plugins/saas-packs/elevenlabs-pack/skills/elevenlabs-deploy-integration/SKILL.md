...
description: |
  Deploy ElevenLabs integrations to production platforms.
  Use when deploying ElevenLabs-powered applications to production,
  configuring platform-specific secrets, or setting up deployment pipelines.
  Trigger with phrases like "deploy elevenlabs", "elevenlabs production",
  "elevenlabs production deploy", "elevenlabs CI/CD".
allowed-tools: Read, Write, Edit, Bash(vercel:*), Bash(fly:*), Bash(gcloud:*)
version: 1.0.0
license: MIT
author: Jeremy Longshore <jeremy@intentsolutions.io>
compatible-with: claude-code
tags: [saas, elevenlabs]
---
...
Deploy ElevenLabs-powered applications as stateless API wrappers — serverless functions
that receive user requests, call the ElevenLabs model API, and stream responses back.
...
export default async function handler(req: Request) {
  const client = new Client({ apiKey: process.env.ELEVENLABS_API_KEY });
