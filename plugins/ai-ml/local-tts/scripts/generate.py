#!/usr/bin/env python3
"""Local TTS via VoxCPM2 — Claude Code plugin helper.
...
def main() -> int:
    ...
    p.add_argument("--ref", help="Reference audio for voice cloning (wav/mp3, 3-10s recommended).")
    p.add_argument("--prompt-wav", help="Prompt wav for ultimate cloning (requires --prompt-text).")
    p.add_argument("--prompt-text", help="Transcript of the prompt wav (required with --prompt-wav).")
    ...
    # Validate prompt-wav requires prompt-text
    if args.prompt_wav and not args.prompt_text:
        print("ERROR: --prompt-wav requires --prompt-text (transcript of the prompt audio)", file=sys.stderr)
        return 2
