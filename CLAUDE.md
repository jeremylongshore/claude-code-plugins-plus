...
**sources.yaml include-anchoring gate:** a step inside the `validate` job (`scripts/check-sources-anchoring.mjs`) fails any PR whose `sources.yaml` include pattern starts with neither `/` (root-anchored) nor `**/` (explicit recursion) — the sync engine silently auto-prefixes such patterns `**/` so they match at ANY depth, which is how an unvetted deep README (carrying a tailnet IP) reached a public sync PR on 2026-07-13. Legacy pre-gate patterns are grandfathered — exactly enumerated, ratchet-only — in `scripts/sources-anchoring-allowlist.txt`: never add rows; anchor the pattern in `sources.yaml` instead, then prune the stale row (the gate warns about prunable rows). The gate reuses `unanchoredIncludes()` from `scripts/sync-lockfile.mjs` — the same predicate the sync engine warns with, so gate and engine cannot disagree.

**Bot-PR token (`BOT_PR_TOKEN`):** the auto-PR workflows (`sync-external.yml`, `promote-curated.yml`, `update-npm-stats.yml`) and the auto-bump push (`auto-bump-on-pr.yml`) authenticate their branch pushes (checkout `token:`) and `gh pr create` with `${{ secrets.BOT_PR_TOKEN || secrets.GITHUB_TOKEN }}`. GitHub suppresses workflow triggers on GITHUB_TOKEN-authored pushes/PRs (anti-recursion), so without the secret every bot PR sits on "Expected" required checks until an owner no-op commit (the 2026-07-13 incident class). Once the owner mints a fine-grained PAT (contents:write + pull-requests:write on this repo) and runs `gh secret set BOT_PR_TOKEN`, bot PRs trigger `ci-required` + `gitleaks` natively; until then behavior is unchanged.
