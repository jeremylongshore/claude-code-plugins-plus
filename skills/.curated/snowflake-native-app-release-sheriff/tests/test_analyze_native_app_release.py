...
FIXTURE_NOW = datetime(2026, 9, 1, 0, 0, tzinfo=timezone.utc)
analyzer._wall_clock_now = lambda: FIXTURE_NOW
    ...
    parser.update(
        {
            ...
            "parse_status": setup["parse_status"],
            "self_contained": setup["self_contained"],
            "fresh_install_tested": setup["fresh_install_tested"],
            "restart_from_beginning_tested": setup["restart_from_beginning_tested"],
            "repeat_run_tested": setup["repeat_run_tested"],
            "upgrade_from_previous_tested": setup["upgrade_from_previous_tested"],
            "test_receipt_sha256": setup["test_receipt_sha256"],
...
class NativeAppReleaseTests(unittest.TestCase):
    def test_clean_qa_fixture_is_ready_without_scan_claim(self):
        ...
        self.assertEqual(report["evaluated_at"], "2026-08-31T23:50:00Z")
    ...
    def test_consumer_review_boolean_mutations_break_artifact_receipt(self):
        cases = (
            (
                "privilege_delta_reviewed",
                lambda data: data["manifest"]["candidate"]["privileges"].append(
                    {
                        "name": "CREATE WAREHOUSE",
                        "description": "Create app warehouse",
                    }
                ),
            ),
            (
                "app_spec_delta_reviewed",
                lambda data: data["manifest"]["candidate"]["app_specs"][0].update(
                    {
                        "sequence": 2,
                        "definition_sha256": "sha256:4444444444444444444444444444444444444444444444444444444444444444",
                    }
                ),
            ),
        )
        for field, create_delta in cases:
            data = load_clean()
            create_delta(data)
            data["manifest"]["consumer_disclosure"][field] = False
            seal_all(data)
    ...
    def test_change_kind_mutation_breaks_artifact_receipt(self):
        ...
        data["package"]["candidate_patch"] = 2
        seal_all(data)
        data["package"]["change_kind"] = "VERSION"
        self.assertIn("ARTIFACT_RECEIPT_UNVERIFIABLE", codes(analyzer.analyze(data)))

    def test_setup_test_boolean_and_digest_mutations_break_parser_receipt(self):
        boolean_fields = (
            "self_contained",
        ...
        for field in boolean_fields:
            data = load_clean()
            data["setup_script"][field] = False
            seal_all(data)
            data["setup_script"][field] = True
            with self.subTest(field=field):
                self.assertIn(
                    "SETUP_PARSER_RECEIPT_UNVERIFIABLE",
                    codes(analyzer.analyze(data)),
                )
    ...
    def test_consistently_backdated_and_resealed_packet_is_wall_clock_stale(self):
        data = load_clean()
        data["as_of"] = "2026-08-31T20:00:00Z"
        ...
        data["cohort_receipt"]["collected_at"] = observed_at
        data["retirement_receipt"]["collected_at"] = observed_at
        data["compatibility"]["collected_at"] = observed_at
        data["rollback"].update({"collected_at": observed_at, "tested_at": observed_at})
        seal_all(data)

        report = analyzer.analyze(data)
        found = codes(report)
        self.assertEqual(report["gate"], "BLOCKED")
        self.assertIn("PACKET_AS_OF_STALE", found)
        self.assertIn("EVIDENCE_WALL_CLOCK_STALE", found)
        self.assertNotIn("EVIDENCE_STALE", found)
