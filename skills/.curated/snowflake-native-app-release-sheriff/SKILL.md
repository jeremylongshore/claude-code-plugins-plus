...
Both packet-relative and current wall-clock freshness limits apply; backdating and
resealing the entire packet cannot make expired evidence current.
...
the stable packet `evaluated_at` copied from `as_of`, and a SHA-256 receipt. The `dry_run_packet`
contains ordered read-only remediation,
