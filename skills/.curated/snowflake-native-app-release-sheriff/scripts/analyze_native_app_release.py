...
REMEDIATION = {
    ...
    "EVIDENCE_WALL_CLOCK_STALE": "Recollect the named surface; packet-relative freshness cannot override wall-clock age.",
    "PACKET_AS_OF_STALE": "Create a new preflight packet with a current UTC as_of and freshly collected receipts.",
...
def _wall_clock_now() -> datetime:
    """Return the production wall clock; tests replace this private seam."""
...
def stale(
    ...
    if observed > wall_clock + timedelta(minutes=1):
        raise ValueError(f"{surface} observation cannot be in the wall-clock future")
    if wall_clock - observed > timedelta(minutes=minutes):
        add_finding(
            findings,
            "EVIDENCE_WALL_CLOCK_STALE",
            surface,
            f"{surface} evidence is older than {minutes} minutes relative to the wall clock",
            "observed",
        )
    ...
    *,
    ...
    try:
        ...
        if collected > wall_clock + timedelta(minutes=1):
            problems.append("collected_at is in the wall-clock future")
        if as_of - collected > timedelta(minutes=60):
            add_finding(
                findings,
                "EVIDENCE_STALE",
                label,
        ...
        if wall_clock - collected > timedelta(minutes=60):
            add_finding(
                findings,
                "EVIDENCE_WALL_CLOCK_STALE",
                label,
                f"{label} receipt is older than 60 minutes relative to the wall clock",
                "observed",
            )
    except ValueError as exc:
...
def validate_source_review(
    ...
    for index, row_value in enumerate(rows):
        ...
        if wall_clock - reviewed_at > timedelta(days=30):
            add_finding(
                findings,
                "EVIDENCE_WALL_CLOCK_STALE",
                f"source_review.{topic}",
                f"official source review for {topic} is older than 30 days relative to the wall clock",
                "observed",
            )
...
def analyze(data: Any) -> dict[str, Any]:
    ...
    findings: list[dict[str, str]] = []
    if as_of > wall_clock + timedelta(minutes=1):
        raise ValueError("as_of cannot be in the wall-clock future")
    if wall_clock - as_of > timedelta(minutes=60):
        add_finding(
            findings,
            "PACKET_AS_OF_STALE",
            "as_of",
            "packet as_of is older than 60 minutes relative to the wall clock",
            "observed",
        )
    ...
    validate_bound_receipt(
        ...
        expected_bindings={
            ...
            "change_kind": normalized_package["change_kind"],
    ...
    validate_bound_receipt(
        ...
        expected_bindings={
            ...
            "candidate_version": normalized_package["candidate_version"],
            "candidate_patch": normalized_package["candidate_patch"],
            "parse_status": setup.get("parse_status"),
            "self_contained": setup.get("self_contained"),
            "fresh_install_tested": setup.get("fresh_install_tested"),
            "restart_from_beginning_tested": setup.get("restart_from_beginning_tested"),
            "repeat_run_tested": setup.get("repeat_run_tested"),
            "upgrade_from_previous_tested": setup.get("upgrade_from_previous_tested"),
            "test_receipt_sha256": setup.get("test_receipt_sha256"),
            "ambiguous": False,
            "executed": False,
        },
        as_of=as_of,
