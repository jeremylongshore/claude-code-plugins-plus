...
- `as_of`: decision timestamp, not in the future and no more than 60 minutes old
  relative to the analyzer wall clock.
...
- `artifact_receipt`: exact candidate/version/patch/change kind, both consumer
  disclosure booleans, previous and candidate
...
version/patch, parse status, all five setup-test booleans, setup test-receipt
SHA-256, and exact statement count. Both `ambiguous` and `executed` must be
...
Source reviews must be no older than 30 days relative to both `as_of` and the
current wall clock. Scan, channel, cohort, retirement, setup, compatibility, and
rollback evidence must be no older than 60 minutes relative to both clocks.
Future timestamps are invalid. Consistently backdated and correctly resealed
receipts still fail the wall-clock maximum-age gate. Remove passwords, tokens,
credentials, private keys, authorization headers, raw SQL, query text, customer
rows/data, PII, and presigned URLs. Evidence collection failures remain blockers.
