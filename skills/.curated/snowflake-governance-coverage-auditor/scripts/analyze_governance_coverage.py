...
SURFACE_CONTRACTS = {
    ...
    "policy_references": [
        ...
        (
            "governance-policy-references-current.sql",
...
def _receipt(
    ...
    if (
        not isinstance(value.get("row_count"), int)
        or isinstance(value.get("row_count"), bool)
...
def _classification_state(
    ...
    if status in {"CLASSIFIED", "REVIEWED"}:
        if last_classified is None:
            ...
            wall_age_hours = (wall_clock - last_classified).total_seconds() / 3600
            if age_hours < 0 or wall_age_hours < 0:
                status = "NOT_PROVEN"
                issues.append("classification timestamp is in the future")
            elif age_hours > max_age_hours or wall_age_hours > max_age_hours:
                status = "STALE"
                issues.append(
                    "classification is stale "
                    f"(assessment age {age_hours:.2f}h, wall-clock age "
                    f"{wall_age_hours:.2f}h, max {max_age_hours:.2f}h)"
                )
...
def analyze(doc: dict, *, now: datetime | None = None) -> dict:
    ...
    if wall_clock.tzinfo is None:
        raise ValueError("current wall clock must include a timezone")
    ...
    preview_features = {
        ...
        for item in _strings(
            metadata.get("preview_features_enabled", []),
            "metadata.preview_features_enabled",
        )
    }
    ...
    assessment_age_hours = (wall_clock - assessed_at).total_seconds() / 3600
    if assessment_age_hours < 0:
        raise ValueError("metadata.assessed_at is after the current wall clock")
    if assessment_age_hours > receipt_max_age:
        raise ValueError(
            "metadata.assessed_at is stale against the current wall clock "
            f"({assessment_age_hours:.2f}h > {receipt_max_age:.2f}h)"
        )
    ...
    for index, row in enumerate(simulations):
        ...
        wall_age_hours = (wall_clock - simulated_at).total_seconds() / 3600
    ...
    for key in sorted(asset_map):
            ...
            findings.append(
                {
                    ...
                    "asset_key": key,
                    "detail": "required governance tag not observed",
                }
            )
        if asset["require_classification"] and classification_state not in {
            "CLASSIFIED",
            "REVIEWED",
        }:
        ...
        if (asset["require_classification"] or asset["required_controls"]) and not edition_ok:
            ...
            findings.append(
                {
                    "severity": "high",
                    "category": "edition-boundary",
                    "asset_key": key,
                    "detail": "Enterprise Edition or higher is required for this governance claim",
                }
            )
            ...
            findings.append(
                {
                    ...
                    "asset_key": key,
                    "detail": "fresh passing POLICY_CONTEXT dry-run not supplied",
                }
            )
        ...
        findings.append(
            {
                ...
                "asset_key": "portfolio",
                "detail": "one or more core evidence receipts are unverifiable",
            }
        )
    ...
    findings.sort(
        key=lambda row: (
            {"critical": 0, "high": 1, "medium": 2}.get(row["severity"], 9),
            row["asset_key"],
            row["category"],
            row.get("policy_kind", ""),
        )
