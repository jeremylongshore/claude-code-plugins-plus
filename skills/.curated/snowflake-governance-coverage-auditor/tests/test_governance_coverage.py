...
from analyze_governance_coverage import analyze as analyze_governance  # noqa: E402
...
SURFACES = {
    "denominator": (
        "governance-denominator.sql",
        "SNOWFLAKE.ACCOUNT_USAGE.TABLES+COLUMNS",
        "assets",
    ),
    "tag_references": (
        "governance-tag-references.sql",
...
FIXTURE_NOW = datetime(2026, 8, 31, 12, 30, tzinfo=timezone.utc)


def analyze(doc: dict) -> dict:
    return analyze_governance(doc, now=FIXTURE_NOW)
...
class GovernanceCoverageTests(unittest.TestCase):
    ...
    def test_current_policy_collector_binds_restricted_manifest_without_exporting_object(
        self,
    ):
            ...
            manifest.write_text(
                json.dumps(
                    [
                        {
                            "asset_key": asset_key,
                            "object_name": "PRIVATE_DB.CORE.CUSTOMERS",
                            "domain": "TABLE",
                        }
                    ]
                ),
                    ...
                    [
                        {
                            ...
                            "POLICY_KIND": "JOIN_POLICY",
                            "ASSIGNMENT": "DIRECT",
                            "POLICY_STATUS": "ACTIVE",
                            "ENTITY_KEY_HASH": None,
                        }
                    ]
                ),
                encoding="utf-8",
    ...
    def test_backdated_resealed_packet_fails_wall_clock_freshness(self):
        doc = load_fixture("covered.json")

        def backdate(value):
            if isinstance(value, dict):
                return {key: backdate(child) for key, child in value.items()}
    ...
    def test_receipt_is_bound_to_exact_dataset_source_template_and_selector(self):
        ...
        doc["tags"].append(
            {
                "asset_key": "asset_customer_email",
                "tag_key": "tag_extra",
                "apply_method": "MANUAL",
            }
        )
    ...
    def test_cli_failure_is_clean_and_cli_output_is_deterministic(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            observed_time = current_time - timedelta(minutes=30)

            def rfc3339(value):
                return value.isoformat().replace("+00:00", "Z")

            doc["metadata"]["assessed_at"] = rfc3339(current_time)
            for receipt in doc["receipts"].values():
                receipt["collected_at"] = rfc3339(observed_time)
            for classification in doc["classifications"]:
                classification["last_classified_on"] = rfc3339(observed_time)
                classification["last_attempt_on"] = rfc3339(observed_time)
            for simulation in doc["policy_context"]:
                simulation["simulated_at"] = rfc3339(observed_time)
            seal_receipts(doc)
